...
func SetupFileManager(t *testing.T) *sql.DB {
	...
	// Todo remove asap for pre-compiled packages.
	dir, err = os.MkdirTemp("", "filemanager")
	require.NoError(t, err)
	t.Cleanup(func() {
	...
	_, err = git.PlainClone(dir, false, &git.CloneOptions{
		URL:      "https://github.com/OrcaBus/service-filemanager",
		Progress: os.Stdout,
	})
	require.NoError(t, err)

	dockerCompose, err := compose.NewDockerComposeWith(compose.WithStackFiles(dir+"/app/compose.yml"), compose.StackIdentifier("filemanager"))
	require.NoError(t, err)
